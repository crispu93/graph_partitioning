...
def custom_loss_equalpart(Y, adj_list, beta):
	'''
	...
	print("D:", D.get_device())
	Gamma = torch.mm(Y.t(), D.unsqueeze(1))
	A = torch.zeros(n,n).to('cuda')
...
def get_edgecut(adj_list, node_idx):
	...
	idx_list2 = [x for a in adj_list for x in a]
	idx = torch.tensor([idx_list1, idx_list2]).to('cuda')
	values = torch.ones(idx.shape[1]).to('cuda')
